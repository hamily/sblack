...
class WeiyouxiClient
{
    ...
    public function __construct( $source , $secret )
    {
        $this->source = $source;
        $this->secret = $secret;
        parse_str($_COOKIE['_GET'],$GET);

        is_array($GET) &&  $_GET = array_merge($GET,$_GET);

        $this->sessionKey = empty( $_GET[ self::PREFIX_PARAM . 'session_key' ] ) ? $_COOKIE['wyx_session_key'] : $_GET[ self::PREFIX_PARAM . 'session_key' ];
        $this->signature = empty( $_GET[ self::PREFIX_PARAM . 'signature' ] ) ? $_COOKIE['wyx_signature'] : $_GET[ self::PREFIX_PARAM . 'signature' ];
