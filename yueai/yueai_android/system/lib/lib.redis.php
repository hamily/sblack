<?php !defined('YUEAI') AND exit('Access Denied!');
...
class Lib_Redis
{
     ...
     * @return Boolean. 如果连接不上或者该key已经存在且不是一个队列
     */
    public function lPushx($key, $value, $zip=false, $serial=true, $timeout=0){
        if( ! $this->connect()){
            return false;
        }
        $value = $serial ? serialize( $value) : $value;
        $value = $zip ? @gzcompress( $value) : $value;
        $flag = $this->oRedis->lPushx($key, $value);
        if($flag && $timeout){
        	$this->oRedis->setTimeout($key,$timeout);
        }
        return $flag;
    ...
    /**
     * 在名称为key的右边（尾）添加一个值为value的元素,如果value已经存在，则不添加
     * @param String $key
     * @param Mixed $value
     * @return Boolean
     */
    public function rPushx($key, $value, $zip=false, $serial=true, $timeout=0){
        if( ! $this->connect()){
            return false;
        }
        $value = $serial ? serialize( $value) : $value;
        $value = $zip ? @gzcompress( $value) : $value;
        $flag = $this->oRedis->rPushx($key, $value);
        if($flag && $timeout){
        	$this->oRedis->setTimeout($key,$timeout);
        }
        return $flag;
     ...
     * @return Mixed
     */
    public function zRevRange($key, $start, $end, $withscores=false, $zip=false, $serial=true){
        if( ! $this->connect()){
            return array();
        }
     ...
     * @param string $start
     * @param string $end
     * @param array $options  array(withscores， limit )
     */
    public function zRangeByScore($key,$start,$end,$options = array()){
        ...
        if(!is_array($options) || empty($options))
            return $this->oRedis->zRangeByScore($key,$start,$end);
        else
            return $this->oRedis->zRangeByScore($key,$start,$end,$options);
    }
    ...
    public function zRevRangeByScore($key,$start,$end,$options = array()){
        ...
        if(!is_array($options) || empty($options))
            return $this->oRedis->zRevRangeByScore($key,$start,$end);
        else
            return $this->oRedis->zRevRangeByScore($key,$start,$end,$options);
    }
    ...
    public function zCount($key,$star,$end){
        ...
        return $this->oRedis->zCount($key,$star,$end);
    }
     ...
     * @return Mized 
     */
    public function zScore($key,$val,$zip=false, $serial=true){
        if( ! $this->connect()){
            return array();
        }
        $val = $serial ? serialize( $val) : $val;
        $val = $zip ? @gzcompress( $val) : $val;
        return $this->oRedis->zScore($key,$val);
    }
    ...
    public function zIncrBy($key,$increment,$val,$zip=false, $serial=true){
        if( ! $this->connect()){
            return array();
        }
        $val = $serial ? serialize( $val) : $val;
        $val = $zip ? @gzcompress( $val) : $val;
        return $this->oRedis->zIncrBy($key,$increment,$val);
    }
     ...
     * @param Mixed $value
     */
    public function zDelete($key, $val, $zip=false, $serial=true){
        if( ! $this->connect()){
            return false;
        }
        return $this->oRedis->zDelete($key, $val);
    }
    /**
     * 返回服务器信息
     * @return Array
    ...
    private function errorlog($keys, $code, $msg){
        ...
        $file = APP_LOG_PATH . 'redis.txt';
        @file_put_contents($file, $error, @filesize($file)<64*1024 ? FILE_APPEND : null);
        die('Redis Invalid!!!');
    }
}