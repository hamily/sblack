...
class Lib_Mysql
{
    ...
    public function connect(){
        ...
        if ($this->persist){
            $this->conn = @mysql_pconnect($this->host, $this->user, $this->password , $this->host.$this->database) or $this->errorlog("連不上數據庫 host:{$this->host} user:{$this->user} pass:{$this->password}");
        }else{
            $this->conn = @mysql_connect($this->host, $this->user, $this->password , $this->host.$this->database) or $this->errorlog("連不上數據庫 host:{$this->host}  user:{$this->user} pass:{$this->password}");
        }        
        if(!is_resource($this->conn)){
            return false;
    ...
    public function escape( $string){
        //$this->open();
        //return mysql_real_escape_string(trim($string), $this->conn);
